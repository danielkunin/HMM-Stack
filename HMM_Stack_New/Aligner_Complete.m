function [Confidence_Band,core_param,stack_param,samples,sites,data] = Aligner_Complete(records)
%
% example
...
% path = 'initial_stack/LR04_short.txt';
...
while ~done
    ...
    parfor index = 1:length(files{1})
        % Forward Algorithm
        [fMatrix,max_T] = forward_algorithm_complete(data,stack_param,core_param,index,rhos);
        tt = ['Forward algorithim for the core ',data(index).name,' in iteration ',num2str(iter),' is done.'];
        disp(tt);
        
        % Backward Sampling Algorithm
        [samples(index,:),sites(index,:)] = back_sampling_complete(fMatrix,sampleSize,data,stack_param,core_param,index,rhos,max_T);
        tt = ['Backward algorithim for the core ',data(index).name,' in iteration ',num2str(iter),' is done.'];
        disp(tt);
    end
    
    % Maximization Step
    core_param = maximization_step_complete(core_param,stack_param,samples,sites,data);
