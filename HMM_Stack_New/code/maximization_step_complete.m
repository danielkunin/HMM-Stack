function [core_param] = maximization_step_complete(core_param_old,stack_param,samples,sites,data)
...
for p = 1:P
    ...
    SUM2 = 0;
    for s = 1:S
        index = (data(p).intervals(:,4) == 0 | data(p).intervals(:,4) == 2);
        AA = sites{p,s};
        SUM1 = SUM1 + sum((data(p).del_O18(:,2)-stack_param.mu(AA(index))'));
        SUM2 = SUM2 + length(AA(index));
    end
...
for p = 1:P
    SUM = 0;
    for s = 1:S
        SUM = SUM + (samples{p,s}(end)-samples{p,s}(1))/(data(p).intervals(end,1)-data(p).intervals(1,1));
    end
