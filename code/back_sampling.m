function [samples] = back_sampling(fMatrix,sampleSize,data,age_stack,rhos)
...
% param: a struct of parameters consisting of the following:
...
% Outputs:
% samples: a L x sampleSize - matrix consisting of samples per each column.


%% Define variables:
T = size(fMatrix,1);
L = size(fMatrix,3);
samples = zeros(L,sampleSize);
depth = data(:,1);
rho_table = rhos{1};
...
while count < sampleSize
    ...
    samples(L,count) = rem(sum_index,T)+1;
    samples(L-1,count) = (sum_index-rem(sum_index,T))/T+1;
    if (samples(L,count)-samples(L-1,count))/(depth(L)-depth(L-1)) <= 4 && (samples(L,count)-samples(L-1,count))/(depth(L)-depth(L-1)) >= 0.25
        det = 1;
        index = L-2;
        while det == 1 && index > 0
            index = (age_stack > age_stack(samples(index+1,count))-4*(depth(index+1)-depth(index)) & age_stack < age_stack(samples(index+1,count))-0.25*(depth(index+1)-depth(index)));
            dummy = 1:T;
            if sum(index) > 1
                possible_t = dummy(index);
                term = fMatrix(possible_t,samples(index+1,count),index+1);
                if sum(isinf(term)) < length(term)
                    term = term - max(term);
                    for t = 1:length(possible_t)
                        term(t) = exp(term(t))*rho_table(1+sum(grid1<(age_stack(samples(index+1,count))-age_stack(possible_t(t)))/(depth(index+1)-depth(index))),sum(grid2<(age_stack(samples(index+2,count))-age_stack(samples(index+1,count)))/(depth(index+2)-depth(index+1))));
                    ...
                    samples(index,count) = possible_t(sum_index+1);
                    index = index - 1;
                else
                    det = 0;
                end
            elseif sum(index) == 1
                ...
                term = fMatrix(possible_t,samples(index+1,count),index+1);
                if isinf(term) == 0
                    samples(index,count) = dummy(index);
                    index = index - 1;
                end
            else
                det = 0;
